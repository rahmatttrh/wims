...
<head>
  ...
  <title>{{ __('Mail') }} {{ __('Checkin') }} {{ $checkin->id }}</title>
</head>

<body style="font-family:Arial,sans-serif;color:#444;padding:1rem;margin:0 auto;max-width:700px;">
  @if ($mail)
    {{ __('Hello') }} {{ $checkin->contact->name }},
    <p style="margin:1rem 0">{{ __('Please review the checkin details below') }}</p>
  @endif
  <div style=" border:1px solid #E5E7EB;border-radius:5px;padding:1rem;line-height:1.4;margin:1rem
    ...
    <div style="display:flex;align-items:flex-start;justify-content:space-between">
      @if ($checkin->warehouse->logo)
        <img src="{{ $checkin->warehouse->logo }}" class="mb-4 sm:mb-0" style="max-height: 90px" />
      @else
        <h1 style="margin:0 1rem 0 0">{{ $checkin->warehouse->name }}</h1>
      @endif
      <div style="text-align:right">
        <div style="font-weight:bold">{{ $checkin->warehouse->name }} ({{ $checkin->warehouse->code }})
        </div>
        <div>{{ $checkin->warehouse->address }}</div>
        <div>{{ $checkin->warehouse->phone }}</div>
        <div>{{ $checkin->warehouse->email }}</div>
      </div>
    </div>
    <div style="border-bottom:1px solid #E5E7EB;margin:1rem -1rem"></div>
    @if ($checkin->draft)
      <div style="border-bottom:1px solid #E5E7EB;margin:-1rem;background:#FDE68A;color:#D97706;padding:0.5rem 1rem">
        {{ __('This record is still a draft.') }}</div>
    @endif
    <div style="width:100%;padding-top:1rem">
      <h1 style="text-align:center;text-transform:uppercase;margin:1rem auto">{{ __('Checkin') }}</h1>
    </div>
    <div style="display:flex;align-items:flex-start;justify-content:space-between;">
      ...
      <div style="width:50%;padding:1rem 0">
        ...
        <div>{{ $checkin->contact->phone }}</div>
        <div>{{ $checkin->contact->email }}</div>
      </div>
    </div>
