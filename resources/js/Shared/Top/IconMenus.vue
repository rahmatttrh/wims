<template>
  <div class="px-4">
    ...
    <Link
      :href="route('settings')"
      v-if="$page.props.auth.user && $page.props.auth.user.roles.find(r => r.name == 'Super Admin')"
      class="inline-flex items-center justify-center p-2 bg-white text-gray-700 hover:text-white hover:bg-blue-700 rounded-sm font-bold"
    >
      <icons name="cog"></icons>
    </Link>
    <button
      @click="alerts = true"
      class="inline-flex items-center justify-center p-2 bg-white text-gray-700 hover:text-white hover:bg-blue-700 rounded-sm font-bold focus:outline-hidden"
    >
