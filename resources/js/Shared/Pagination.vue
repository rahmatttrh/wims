  ...
  <div v-if="meta.total > 0">
      ...
      <div v-if="meta.last_page > 1" class="flex flex-wrap -mb-1">
        <template v-for="(link, i) in meta.links" :key="'link_' + i">
          <template v-if="link.url === null">
            <div
              class="mr-1 mb-1 px-4 py-3 text-sm leading-4 text-gray-400 shadow-sm opacity-50 bg-white rounded-md cursor-default"
              v-html="
          ...
          <template v-else>
            <tec-button
              ...
              class="inline-block whitespace-nowrap mr-1 mb-1 px-4 py-3 text-sm leading-4 shadow-sm rounded-md hover:bg-blue-700 hover:text-white hover:border-blue-700 focus:border-blue-600 focus:text-white"
            ></tec-button>
          </template>
        </template>
      </div>
    ...
    <div class="flex items-center justify-between xl:hidden">
      <tec-button
        ...
        class="inline-block whitespace-nowrap mr-1 mb-1 px-4 py-3 text-sm leading-4 shadow-sm rounded-md print:hidden"
        :class="
          links.prev
