<template>
  <admin-layout :title="$t('Resi Checkout')">
    <div class="px-4 md:px-0">
      <div class="flex items-start justify-between print:hidden">
        <tec-section-title class="-mx-4 md:mx-0 mb-6">
          <template #title>
            <div class="flex items-center">
              <Link class="text-blue-600 hover:text-blue-700" :href="route('checkouts.index')">{{ $t('Checkouts') }}</Link>
              <span class="text-blue-600 font-medium mx-2">/</span>
              {{ checkout.reference }}
            </div>
          </template>
          <template #description>{{ $t('Please review the outbound receipt below') }}</template>
        </tec-section-title>
        <div class="flex">
          <button
            @click="print()"
