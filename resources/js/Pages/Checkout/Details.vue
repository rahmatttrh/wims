<template>
  ...
  >
    <div class="bg-white p-4 rounded-md shadow-sm overflow-x-auto print:shadow-none print:pt-0">
      ...
      <trashed-message v-if="checkout.draft == 1" class="mb-4 h-12 print:hidden" :action="false" icon="info">
        {{ $t('This record is still a draft.') }}
      </trashed-message>
      <trashed-message v-if="checkout.deleted_at" class="mb-4 h-12 print:hidden" :action="false">
      ...
      <div class="block sm:flex justify-between print:flex">
        ...
        <div class="text-left w-full sm:w-1/2 leading-snug">
          ...
          <div v-if="checkout.contact.phone">{{ checkout.contact.phone }}</div>
          <div v-if="checkout.contact.email">{{ checkout.contact.email }}</div>
        </div>
      </div>
        ...
        <table class="w-full mt-8 mb-4" style="min-width: 500px">
          ...
          <template v-for="(item, ii) in checkout.items" :key="'i_' + item.id">
            <tbody
              ...
              :class="{ 'bg-gray-50': ii % 2 != 0, 'border-b': checkout.items.length == ii + 1 }"
            >
              <tr>
            ...
            <tbody v-else class="group avoid" :class="{ 'bg-gray-50': ii % 2 != 0, 'border-b': checkout.items.length == ii + 1 }">
              <tr>
      ...
      <div v-if="checkout.attachments && checkout.attachments.length" class="print:hidden py-4 w-full">
        <dt class="font-medium">{{ $t('Attachments') }}</dt>
        <dd class="mt-1 text-sm text-gray-900">
          <ul role="list" class="border border-gray-200 rounded-md divide-y divide-gray-200">
            <li
              v-for="attachment in checkout.attachments"
              :key="attachment.id"
      ...
      <div v-if="checkout.details" class="py-4 w-full">
        {{ $t(checkout.details) }}
      </div>
    </div>
...
export default {
  ...
  mounted() {
    ...
    JsBarcode('#barcode', this.checkout.reference, {
      width: 1,
      margin: 0,
      fontSize: 0,
      height: '20',
