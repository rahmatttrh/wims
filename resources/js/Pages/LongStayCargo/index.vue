<template>
    <admin-layout :title="$t('Long Stay Cargo')">
      <div class="px-4 md:px-0">
        <tec-section-title class="-mx-4 md:mx-0 mb-6">
          <template #title>{{ $t('Long Stay Cargo') }}</template>
    ...
    watch: {
      form: {
        handler: throttle(function () {
          ...
          this.$inertia.get(
            this.route('longstaycargo.index', Object.keys(query).length ? query : { remember: 'forget' }),
            {},
            {
              onFinish: () => {
