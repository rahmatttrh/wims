<template>
  <admin-layout :title="$t('import_x', { x: $t('Categories') })">
    <tec-form-section @submitted="submit">
      <template #title>{{ $t('import_x', { x: $t('Categories') }) }}</template>
          ...
          >
            <div class="space-y-1 text-center">
              ...
              <div class="text-xs text-gray-700">
                <div>{{ $t('Excel file should have name, code & parent columns.') }}</div>
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      this.form.post(route('categories.import.save'), { preserveScroll: true });
    },
  },
};
</script>
