<template>
  <admin-layout :title="$t('import_x', { x: $t('Units') })">
    <tec-form-section @submitted="submit">
      <template #title>{{ $t('import_x', { x: $t('Units') }) }}</template>
          ...
          >
            <div class="space-y-1 text-center">
              ...
              <div class="text-xs text-gray-700">
                <div>{{ $t('Excel file should have name, code, base_unit, operator & operation_value columns.') }}</div>
                <div>
                  {{ $t('You must fill the name and code columns.') }} {{ $t('base_unit is the code of the base unit.') }}
                  {{ $t('operator could be `*`, `/`, `+` & `-`') }}
                </div>
              </div>
...
export default {
  ...
  // computed: {
  //   selected() {
  //     return this.$refs.file && this.$refs.file.files && this.$refs.file.files[0] ? this.$refs.file.files[0].name : null;
  //   },
  // },
  ...
  methods: {
    ...
    submit() {
      ...
      // this.$inertia.post(route('units.import.save'), data);
      this.form.post(route('units.import.save'), { preserveScroll: true });
    },
  },
};
</script>
