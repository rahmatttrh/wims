...
class CheckinExport implements FromCollection, WithHeadings, WithMapping, WithEvents, WithDrawings, WithStyles
{
    protected $filters;
    ...
    public function map($checkin): array
    {
        return [
            ++$this->rowNumber,
            $checkin->reference,
            $checkin->date ? Carbon::parse($checkin->date)->format('Y-m-d') : '',
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->setCellValue('C1', 'INBOUND REPORT');
